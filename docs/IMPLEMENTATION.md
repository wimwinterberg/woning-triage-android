...
- **We Create Solutions Address API** (`GET https://address-api.createsolutions.dev/v1/postcode/{postalCode}/{houseNumber}`, Bearer `WCS_ADDRESS_API_KEY`). Alleen **Nederland** (`country=nl`); BE/DE-resultaten en kandidaten met een andere postcode of huisnummer worden genegeerd. 200 geeft kandidaten; 404 is een lege lijst; 401/429/503/500 is `address_lookup_unavailable`. Meerdere units komen via `houseLetter` / `houseNumberAddition`. De API heeft geen reverse-geocode-route; GPS in de app stuurt lat/lon plus nabije postcode/huisnummer-hints (van de telefoon-geocoder). De backend zoekt die hints via WCS op en toont de kandidaten. De bewoner moet zelf tikken; GPS verifieert nooit automatisch. Fake-resultaten (`Voorbeeldstraat`) mogen niet als live BAG worden gepresenteerd. Address-lookup logs gaan naar STDERR (`docker compose logs -f api live-gateway`) zonder postcode, huisnummer, straat of coördinaten. Bij “nee” / verkeerde postcode tijdens bevestigen wordt de lookup gewist en opnieuw gevraagd.
